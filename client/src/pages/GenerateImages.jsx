...
const GenerateImages = () => {
  const imageStyle = [
      'Realistic', 'Cartoon', 'Anime style', 'Ghibli style', 'Fantasy style', 'Futuristic style', 'Portrait style', '3D style', 'Nature', 'Urban'
  ]
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item) => (
            <span onClick={()=> setSelectedStyle(item)}
             className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle.text === item.text ? 'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span>
          ))}
        </div>
