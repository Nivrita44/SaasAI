...
const CreationItem = ({item}) => {
  ...
  return (
    <div onClick={()=> setExpanded(!expanded)} className='p-4 max-w-5xl text-xm bg-white border border-gray-200 rounded-lg cursor-pointer'>
      ...
      </div>
      {
        expanded && (
            <div>
                {item.type === 'image' ? (
                  <div>
                    <img src={item.image} alt='image' className='mt-3 w-full max-w-md '/> 
                    </div>  
                ) : (
